...
/**
 * 通知 service — 创建通知并通过 SSE 实时推送的领域逻辑
 *
 * 这部分逻辑从 `routes/notifications.ts` 抽出，供 server 内其它 service / module
 * （generation、subtitle、gateway、auth）复用，避免「业务模块反向 import 路由」
 * 的层级倒置（路由只应被 index.ts 挂载，业务逻辑不应依赖路由层）。
 *
 * HTTP CRUD（列表 / 未读数 / 标记已读）仍留在 `routes/notifications.ts`。
 */
