 ...
 */
import type { CanvasProjectListResponse, CanvasProjectResponse, CanvasProjectSummaryResponse, CanvasAssetsPollResponse, CanvasPipelineRunListResponse, CanvasPipelineRunResponse, CanvasMutationOkResponse } from '@excuse/shared'
...
export async function handleGetAssetsPoll(projectId: string, userId: string) {
  ...
  if (!poll) throw new NotFoundError('项目不存在')
...
export async function handlePatchProject(projectId: string, userId: string, body: { title?: string, storyText?: string }) {
  ...
  if (!owned) throw new NotFoundError('项目不存在或无权访问')
  const { title, storyText } = body
  if (title === undefined && storyText === undefined) throw new ValidationError('至少提供一个字段')
...
export async function handleGetRun(runId: string, userId: string) {
  ...
  if (!run) throw new NotFoundError('运行记录不存在')
