...
- ✅ 跨业务状态联动修复 — **取消级联** + **重排级联**（对偶设计）。取消级联：`cancelAdminTask` 取消任务后，若 `task.generationRecordId` 非空，经 `cancelGenerationRecordIfActive` 级联取消关联 `generation_records`——仅非终态 `pending/submitting/processing/saving_output` 才置 `cancelled`，修复 queued 态取消后记录滞留 `processing` 的悬挂态 bug；running 态取消时若 worker 已 `markGenerationSucceeded` 则跳过、不覆盖成功产物（commit: `3c83220`）。重排级联：`requeueAdminTask` 为其对偶，经 `requeueGenerationRecordIfRequeueable` 仅重置终态 `failed/cancelled` 记录为 `pending`（清 errorMessage / 递增 retryCount / 清 dedupeKey），使重跑窗口内 UI 反映"正在重试"，活跃态与 succeeded 不动避免回退/覆盖；两者均 best-effort `.catch` 静默降级（commit: `8758940`）。
