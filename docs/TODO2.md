...
| 优先级 | 待办 | 条目 | 预估工作量 |
|---|---|---|---|
| P0 立刻 | unwrapEden 401 登录态清理 | §4.1 | 0.5h |
| P0 立刻 | 安全响应头 | §5.1 | 0.5h |
| P1 短期 | 错误反馈分级策略 | §2.1 | 4h |
| P1 短期 | client.ts 拆分 | §3.1 | 3h |
| P1 短期 | workspace store 拆分 | §3.2 | 4h |
| P1 短期 | Admin refetchInterval 可见性控制 | §4.2 | 1h |
| P1 短期 | 服务端空 catch 块补 log | §4.3 | 1h |
| P2 中期 | MediaPreviewDialog 支持视频/音频 | §1.1 | 1h |
| P2 中期 | 忘记密码页面 | §1.2 | 2h |
| P2 中期 | ModelLab 离开确认 | §2.2 | 1h |
| P2 中期 | 全局 React Query 错误处理 | §4.4 | 2h |
| P2 中期 | Canvas loadProject 去重 | §4.5 | 1h |
| P2 中期 | Auth cookie 安全加固 | §5.2 | 1h |
| P2 中期 | SubtitleEditor 拆分 | §3.3 | 2h |
| P2 中期 | 前端错误监控 | §6.2 | 2h |
| P3 优化 | Billing 刷新按钮修复 | §2.3 | 5min |
| P3 优化 | 密码可见性切换 | §1.3 | 0.5h |
| P3 优化 | 网络状态指示器 | §2.4 | 1h |
| P3 优化 | 表单成功反馈 | §2.5 | 0.5h |
| P3 优化 | Canvas useEffect 修复 | §4.6 | 0.5h |
| P3 优化 | eslint-disable 清理 | §4.7 | 2h |
| P3 优化 | Admin props 重构 | §3.4 | 1h |
| P3 优化 | Query key 统一 | §3.5 | 1h |
| P3 优化 | 重置密码专项限流 | §5.3 | 1h |
| P3 优化 | Elysia body size limit | §5.4 | 0.5h |
| P3 优化 | 登录账号锁定 | §5.5 | 2h |
| P3 优化 | Docker 缓存优化 | §6.3 | 2h |
| P3 优化 | 前端性能监控 | §6.1 | 2h |
...
**建议执行顺序**：
1. **本周**（P0）：§4.1 unwrapEden 401 修复 + §5.1 安全头 → 30 分钟搞定，消除两个生产风险
2. **下周**（P1）：§2.1 错误分级 + §3.1 client.ts 拆分 + §4.2 Admin refetch → 提升体验和可维护性
3. **本月**（P2/P3）：其余功能缺失 + 安全加固 + 监控集成
